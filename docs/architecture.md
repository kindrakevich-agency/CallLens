...
so the whole pipeline runs end-to-end with no paid API calls. Real Deepgram / OpenAI
clients are **Planned (M3–M5)**.

The `scheduler` service consumes `scheduler_default` for housekeeping
(retention sweeps are **Planned (M8)**).
